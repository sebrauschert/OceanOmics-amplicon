...
# assays = c("16S", "MiFish")
# sites = c()
# metadata <- read_csv(paste0("05-report/RSV5_metadata.csv"))
# sites <- metadata$`Sample ID`
...
dada2_analysis1 <- function(voyage = voyage, 
  ...
  ggsave(plot = errorsplot_Fs, 
         filename = paste0("03-dada2/QC_plots/", voyage, "_errorsplot_Fs_", assay,"_", site,".png"), 
         height = 5, 
         width = 7)
  errorsplot_Rs <- plotErrors(errors_reverse, nominalQ = TRUE)
  ggsave(plot = qualityprofile_Rs, 
         filename = paste0("03-dada2/QC_plots/", voyage, "_errorsplot_Rs_", assay,"_", site, ".png"), 
         height = 5, 
         width = 7)
